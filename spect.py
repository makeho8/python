...
df_train = pd.read_csv('http://archive.ics.uci.edu/ml/machine-learning-databases/spect/SPECT.train')
df_test = pd.read_csv('http://archive.ics.uci.edu/ml/machine-learning-databases/spect/SPECT.test')
...
print('accuracy of WS_SVM: %s' %(100*np.mean(y_pred_WS_SVM==y_test)), clf2.score(AB_test, y_test))
###Cross validation score of WS_SVM
from sklearn.model_selection import cross_val_score
scores = cross_val_score(estimator = clf2, X = AB_train, y = y_train, cv = 10, n_jobs =1)
...
#from sklearn.model_selection import cross_val_score
scores = cross_val_score(estimator = clf3, X = AB_train, y = y_train, cv = 10, n_jobs =1)
...
gs2 = gs2.fit(AB_train, y_train)
#print('best score of WS_SVM: ', gs2.best_score_) 
print('best params of WS_SVM', gs2.best_params_)
clf2 = gs2.best_estimator_
clf2.fit(AB_train, y_train)
print('Test accuracy of WS_SVM: %.3f' % clf2.score(AB_test, y_test))
scores = cross_val_score(estimator = clf2, X = AB_train, y = y_train, cv = 10)
#print('CV accuracy scores of WS_SVM: %s' %scores)
print('CV accuracy of WS_SVM: %.3f +/- %.3f' % (np.mean(scores), np.std(scores)))
...
gs3 = gs3.fit(AB_train, y_train)
#print('best score of S_TWSVM: ', gs3.best_score_) 
print('best params of S_TWSVM', gs3.best_params_) 
clf3 = gs3.best_estimator_
clf3.fit(AB_train, y_train)
print('Test accuracy of S-TWSVM: %.3f' % clf3.score(AB_test, y_test))
scores = cross_val_score(estimator = clf3, X = AB_train, y = y_train, cv = 10)
...
gs4 = gs4.fit(AB_train, y_train)
#print('best score of TWSVM: ', gs4.best_score_)
print('best params of TWSVM', gs4.best_params_)
clf4 = gs4.best_estimator_
clf4.fit(AB_train, y_train)
print('Test accuracy of TWSVM: %.3f' % clf4.score(AB_test, y_test))
scores = cross_val_score(estimator = clf4, X = AB_train, y = y_train, cv = 10)
print('CV accuracy of S-TWSVM: %.3f +/- %.3f' %(np.mean(scores), np.std(scores)))