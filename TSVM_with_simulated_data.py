...
#A, yA, B, yB, C, y = gen_lin_separable_data()
A, yA, B, yB, C, y = gen_lin_separable_overlap_data()
#A, yA, B, yB, C, y = gen_non_lin_separable_data()

# Minh hoa du lieu
with PdfPages('data.pdf') as pdf:
    plt.plot(A[:,0], A[:,1], 'ks', markersize = 8, alpha = 1)
    plt.plot(B[:,0], B[:,1], 'ko', markersize = 8, alpha = .5)
    plt.axis('tight')
...
def myplot(A, B, w, b, filename, tit):
    with PdfPages(filename) as pdf:
        ...
        plt.plot(A[:,0],A[:,1], 'ks', markersize = 8, alpha = .8)
        plt.plot(B[:,0],B[:,1], 'ko', markersize = 8, alpha = .4)
        pdf.savefig()
        plt.show()

import time
start_time = time.time()
w_dual, b_dual = SVM_solver(A, B, c=1)
end_time = time.time()
...
def myplot12(A, B, w1, b1, w2, b2, filename, tit):
    with PdfPages(filename) as pdf:
        ...
        plt.plot(x11, y12, 'k')
        plt.plot(x21, y21, 'gray', linewidth = 3)
        plt.plot(x21, y22, 'gray')
...
start_time = time.time()
w1_tsvm, b1_tsvm, w2_tsvm, b2_tsvm = TSVM_solver(A, B, c1=1.5, c2=1.5)
end_time = time.time()
...
def plot_contour(A, B):
    ...
    Z1 = (Kernel(X,C.T,1).dot(u1_Ktsvm)+b1_Ktsvm).reshape(X1.shape)
    Z2 = (Kernel(X,C.T,1).dot(u2_Ktsvm)+b2_Ktsvm).reshape(X1.shape)
    plt.contour(X1, X2, Z1, [0.0], colors='k', linewidths=3, origin='lower')
    plt.contour(X1, X2, Z2 , [0.0], colors='grey', linewidths=3, origin='lower')
    plt.title('Nghiệm tìm bởi rbf-kernel TSVM')
    plt.axis('tight')
    #plt.axis('equal')
    #plt.xlim([-7,7])
    #plt.ylim([-7,7])
...
A, yA, B, yB, C, y = gen_non_lin_separable_data()
start_time = time.time()
u1_Ktsvm, b1_Ktsvm, u2_Ktsvm, b2_Ktsvm = KTSVM_solver(A, B, c1=.1, c2=.1)
end_time = time.time()

plot_contour(A, B)
print('Total run-time of KTSVM: %f ms' %((end_time - start_time)*1000))
