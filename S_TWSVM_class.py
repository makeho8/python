...
class S_TWSVM(BaseEstimator):
    
    def __init__(self, kernel = None, polyconst =1, degree = 2, gamma = 10,ci=None, ci_=None, c=None, c_=None):
        self.kernel = kernel
    ...
    def fit(self, X, y):
        ...
        if self.ci_ == None:
            ...
            K = matrix(GB.dot(np.linalg.inv(HA.T.dot(HA)+ self.ci_*J + 0.0001*I)).dot(GB.T))
        ...
        if self.ci_ == None:
            ...
            self.u = -np.linalg.inv(((HA.T).dot(HA)+ self.ci_*J + 0.0001*I)).dot(GB.T).dot(alpha)
        ...
        if self.c_ == None:
            ...
            K = matrix(HA.dot(np.linalg.inv(GB.T.dot(GB)+ self.c_*F + 0.0001*I)).dot(HA.T))
        ...
        if self.c_ == None:
            ...
            self.v = -np.linalg.inv(GB.T.dot(GB)+ self.c_*F + 0.0001*I).dot(HA.T).dot(beta)
