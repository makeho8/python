...
df_votes = pd.read_csv('https://archive.ics.uci.edu/ml/machine-learning-databases/voting-records/house-votes-84.data', header = None)

from sklearn.impute import SimpleImputer

imr = SimpleImputer(missing_values = '?', strategy = 'most_frequent')
imr1 = imr.fit(df_votes)
df = imr1.transform(df_votes)
df1 = pd.DataFrame(df)
size_mapping = {'y':1, 'n':0}
for i in range(1,df_votes.shape[1]):
    df1[i] = df1[i].map(size_mapping)
    
X = df1.iloc[:,1:].values
y = df1.iloc[:,0].values
A = X[np.where(y == 'republican')]
B = X[np.where(y == 'democrat')]
y_A = np.ones(len(A))
y_B = -np.ones(len(B))
AB = np.vstack((A,B))
