...
df_diabetis = pd.read_csv("D:/XITRUM/nghiên cứu sinh/SVMs and improvements/diabetes_csv.csv")
X, y = df_diabetis.iloc[:,:8].values, df_diabetis.iloc[:,8]
A = X[np.where(y == 'tested_positive')]
B = X[np.where(y == 'tested_negative')]
y_A = np.ones(len(A))
y_B = -np.ones(len(B))
AB = np.vstack((A,B))
...
###WS_SVM best params of WS_SVM {'c1': 1.0, 'c2': 0.001, 'c3': 0.1}
from WS_SVM_class import WS_SVM
start_time = time.time()
clf2 = WS_SVM(c1 = 1, c2 = 0.001, c3 = 0.1)
...
### S_TWSVM best params of S_TWSVM {'c1': 1.0, 'c2': 0.0001, 'c3': 0.0001}
from S_TWSVM_class import S_TWSVM
start_time = time.time()
clf3 = S_TWSVM(c1 = 1, c2 = 0.0001, c3 = 0.0001)
...
# TWSVM best params of TWSVM {'c': 1, 'c_': 1}
